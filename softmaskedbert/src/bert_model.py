...
class BertLMPredictionHead(nn.Cell):
    def __init__(self, config):
        super().__init__()
        # self.transform = BertPredictionHeadTransform(config) # gil 20220316
        ...
        # self.decoder = nn.Dense(config.hidden_size, config.vocab_size, bias_init="zeros", has_bias=False) # gil 20220316
    ...
    def construct(self, hidden_states):
        # hidden_states = self.transform(hidden_states) # gil 20220316
        # hidden_states = self.decoder(hidden_states) # gil 20220316
...
class BertPredictionHeadTransform(nn.Cell):
    def __init__(self, config):
        ...
        # self.LayerNorm = nn.LayerNorm([config.hidden_size], epsilon=config.layer_norm_eps) # gil 20220316

    def construct(self, hidden_states):
        hidden_states = self.dense(hidden_states)
        hidden_states = self.gule(hidden_states)
        hidden_states = self.layernorm(hidden_states)
        # hidden_states = self.LayerNorm(hidden_states) # gil 20220316
        return hidden_states
...
#
#     def construct(self,  word_embeddings, token_type_ids=None):
#         """Postprocessors apply positional and token type embeddings to word embeddings."""
#         # if token_type_ids == None:
#         #     shape = word_embeddings.shape[:-1]
#         #     zeros = ops.Zeros()
#         #     token_type_ids = zeros(shape, mstype.int32)
...
class RelaPosEmbeddingsGenerator(nn.Cell):
    ...
    def construct(self):
        ...
        if self.use_one_hot_embeddings:
            ...
            # embeddings = self.matmul(one_hot_relative_positions_matrix.astype("float16"), self.embeddings_table.astype("float16")) # can use float32 if mindspore version >= 1.5.0
...
class BertAttention(nn.Cell):
    ...
    def construct(self, from_tensor, to_tensor, attention_mask):
        ...
        # attention_scores = self.matmul_trans_b(query_layer.astype("float16"), key_layer.astype("float16")) # can use float32 if mindspore version >= 1.5.0
        ...
        if self.use_relative_positions:
            ...
            # key_position_scores = self.matmul_trans_b(query_layer_r.astype("float16"),
            #                                           relations_keys.astype("float16")) # can use float32 if mindspore version >= 1.5.0
        ...
        # context_layer = self.matmul(attention_probs.astype("float16"), value_layer.astype("float16")) # can use float32 if mindspore version >= 1.5.0
        ...
        if self.use_relative_positions:
            ...
            # value_position_scores = self.matmul(attention_probs_r.astype("float16"),
            #                                     relations_values.astype("float16")) # can use float32 if mindspore version >= 1.5.0
...
class CreateAttentionMaskFromInputMask(nn.Cell):
    ...
    def construct(self, input_mask):
        ...
        # debug2 = cast(input_mask, mstype.float32)
