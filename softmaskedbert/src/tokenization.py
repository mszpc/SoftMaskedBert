...
# def set_tokenizer2():
#     vocab = text.Vocab.from_file('./src/bert-base-chinese-vocab.txt')
...
class CscTokenizer:
    def __init__(self, device_num=1, rank_id=0, fp="", max_seq_len=0, vocab_path='./src/bert-base-chinese-vocab.txt'):
        ...
        # self.convert2id = self.convert2id()
    ...
    def set_tokenizer(self):
        # import sys
        # print('set_tokenizer: ' + str(sys.path[0]))
    ...
    def convert2det_labels(self, wrong_ids_ndarray, tokens_ndarray):
        ...
        #print(tokens_list_str)
        #print(det_labels_list)
        # pad to max_seq_len
    ...
    def get_token_ids(self, batch_size):
        ...
        #for data in dataset.create_dict_iterator(num_epochs=1, output_numpy=True):
            #print(data['original_tokens'])
        ## column_names=['original_text', 'original_tokens', 'wrong_ids', 'correct_text', 'correct_tokens'])
        ## dataset = dataset.map(operations=self.get_worng_ids_ndarray, input_columns=['wrong_ids'])
        #dataset = dataset.map(operations=self.tokenizer_op, input_columns=['original_tokens'])
        dataset = dataset.map(operations=self.tokenizer_op, input_columns=['original_tokens'])
        ## create det_labels_list
        ## dataset = dataset.map(operations=self.convert2det_labels, input_columns=['wrong_ids', 'original_tokens'],
        ##                       output_columns=['wrong_ids'],
        ##                       column_order=['wrong_ids', 'original_tokens', 'correct_tokens'])
...
if __name__ == '__main__':
    ...
    # dataset = get_dataset(fpath, tokenizer_op)
    # count = 0
    # for data in dataset.create_dict_iterator(num_epochs=1, output_numpy=True):
    #     print(text.to_str(data['original_text']))
    #     count = count + 1
    #     if count > 3:
    #         break
    # print('==============================================================================')
    # print('==============================================================================')
    # dataset = dataset.map(operations=tokenizer_op, input_columns=['original_text'])
    # count = 0
    # for data in dataset.create_dict_iterator(num_epochs=1, output_numpy=True):
    #     print(text.to_str(data['original_text']))
    #     count = count + 1
    ...
    for data in dataset1.create_dict_iterator(num_epochs=1, output_numpy=True):
        ...
        # print(convert2id(data['original_text'], demo.vocab))
        #print("========================================")
        count1 = count1 + 1
        if count1 > 3:
            break
