...
def do_train(dataset, network, cfg, profile=None, save_ckpt_path='./checkpoint', epoch_num=1):
    ...
    ckpoint_cb, time_cb], dataset_sink_mode=True)
    if cfg.enable_modelarts:
...
def run_csc():
    ...
    parser.add_argument("--baselr", type=float, default=0.000005) # 0.0001
    ...
    if args_opt.enable_modelarts:
        tokenizer = CscTokenizer(fp=local_train_file_dir, device_num=device_num, rank_id=rank_id, \
